...
public class DataEditor
{
    ...
    public static string CsvPath = Application.dataPath + "/../Data/Csv/";
    ...
    public static string OLDDLLPATH = Application.dataPath + "/../HotFix/HotFix/bin/Debug/HotFix.dll";
    ...
    public static void AllEditorToXml()
    {
        ...
        for (int i = 0; i < filePaths.Length; i++)
        {
            ...
            Debug.LogError("查找文件夹下的类,正在扫描路径" + name + "     " + filePaths[i]);
            ExcelToXml(name);
        }
        AssetDatabase.Refresh();
        EditorUtility.ClearProgressBar();
    }
    [MenuItem("Tools/Xml/Csv转Json")]
    public static void AllCsvToJson()
    {
        string[] filePaths = Directory.GetFiles(CsvPath, "*", SearchOption.AllDirectories);
        for (int i = 0; i < filePaths.Length; i++)
        {
            if (!filePaths[i].EndsWith(".csv"))
                continue;
            EditorUtility.DisplayProgressBar("查找文件夹下的csv文件", "正在扫描路径" + filePaths[i] + "... ...", 1.0f / filePaths.Length * i);
            string name = Path.GetFileNameWithoutExtension(filePaths[i]);
            Debug.LogError("查找文件夹下的类,正在扫描路径" + name + "     " + filePaths[i]);
            CsvToJson(name);
        }
    ...
    private static void CsvToJson(string name)
    {
    }
